...
# %% Import the data
with h5py.File('./training_datasets_pyrocko_Z.hdf5', 'r') as f:
    time = f['time'][:]
...
plt.show()
plt.savefig('./Figures/Loss_evolution_38.png')

# %% Save the model
model_dataset_dir = './Model_and_datasets'
if not os.path.exists(model_dataset_dir):
    os.mkdir(model_dataset_dir)
model.save(model_dataset_dir + '/Synthetic_seismogram_Z_Conv1DTranspose_38.hdf5')

# %% Save the datasets
model_datasets = model_dataset_dir + '/Synthetic_seismogram_Z_Autoencoder_model_datasets.hdf5'
